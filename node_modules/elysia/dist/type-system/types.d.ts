import type { Kind, ObjectOptions, SchemaOptions, StaticDecode, TObject, TProperties, TransformKind, TSchema, TUnsafe, Uint8ArrayOptions } from '@sinclair/typebox';
...
export type ElysiaTypeCustomError = string | boolean | number | ElysiaTypeCustomErrorCallback;
export type ElysiaTypeCustomErrorCallback = (error: {
     ...
     */
    errors: ValueError[];
}, validator: TypeCheck<any>) => unknown;
...
export type AssertNumericEnum<T extends Record<string, string | number>>= {
    [K in keyof T]: K extends number ? string : K extends `${number}` ? string : K extends string ? number : never;
};
